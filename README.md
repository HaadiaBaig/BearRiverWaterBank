...
5.  The users make decisions to consume, conserve, or trade water based on historic water use, which is used in the model as a proxy to the water they are entitled to use.
...
1.  The model contains
2.  On the Model worksheet, scroll down Column B. The instructions are given in rows with Blue text. For example, in **Rows 6-11**, participants select a User and enter the User's Strategy to participate in the banking. If fewer than 5 participants, participants select multiple parties.
    -   Sample strategies
...
3.  In **Cell 24**, select the Bear Lake starting level for start of the water year.
...
9.  **Row 47-51**, the model calculates the natural flow from April – September for each user.
10. **Row 53-57**, the model calculates the historic depletions by each user.
...
13. Model allocates available natural flow to each user, calculated by the formula
...
16. In Year 2 – 4 User decides is they want to use any water from conserved storage from previous years.
...
21. User sets or negotiates the price of water (\$/acft) with the bank. [(See benchmarks for pricing.](https://github.com/HaadiaBaig/BearRiverWaterBank/blob/main/AdditionalInformation/Pricing.md#pricing))
